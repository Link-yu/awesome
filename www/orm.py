...
def select(sql, args, size=None):
    ...
    with (yield from __pool) as conn:
        ...
        conn.close()
        logging.info('row return: %s' %(len(rs.result())))
        return rs.result()
...
# -*-定义Model的元类
# 所有的元类都继承字type
# ModelMetaclass 元类定义了所有Model基类（继承ModelMetaclass）的子类实现的操作

# -*-ModelMetaclass的工作主要是为了一个数据库表映射程一个封装的类做准备：
# ***读取具体子类（user）的映射信息
# 创造类的时候，排除对Model类的修改
# 在当前类中查找所有的类属性（attrs），如果找到Field属性，就将其保存到__mappings__的dict中，同时从类属性中删除Field(防止实例属性遮住类的同名属性)
# 将数据库表明保存到__table__中

# 完成这些工作后就可以在Model中定义各种数据库的操作方法

class ModelMetaclass(type):
    #__new__控制__init__的执行，所以在其执行之前
    # cls：代表要__init__的类，次参数在实例化时由Python解释器自动提供
    # bases:代表继承父类的集合
    # attrs:类的方法集合
    def __new__(cls, name,bases,attrs):
        ...
        # end for
        if not primary_Key:
            raise StandardError('Parimary key is not found')
...
# 基于字典查询形式
# Model从dict继承，拥有字典的所有概念，同时实现特殊方法__getattr__和__setattr__，能够实现属性操作
...
class Model(dict,metaclass=ModelMetaclass):
    ...
    @classmethod
    # 类方法有类变量cls传入，从而可以用cls做一些相关的处理。并且有子类继承，调用该类方法
    @asyncio.coroutine
